...
def gpt_response(prompt):
    GPT_key = get_parameter('CHATGPT_KEY') # API key stored in Parameter Store (will also not work)
    ...
    if res.status_code != 200:
        raise ValueError(f"Failed to get a response from the OpenAI API, status code: {res.status_code}, response text: {res.text}")
    
    response_json = res.json()
    
    # Check if 'choices' is in the response
    if 'choices' not in response_json:
        raise ValueError(f"'choices' not in response: {response_json}")
...
def upload_to_cloudinary(filename, resource_type, fields=None):
    Cloudinary_key = get_parameter('CLOUDINARY_KEY') # API key stored in Parameter Store (will also not work)
    # Need to put own info here
    api_key = ""
    cloud_name = ""
